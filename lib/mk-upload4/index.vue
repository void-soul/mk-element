      ...
      <div v-loading="chooseLoading"
        ...
        <el-row v-loading="chooseNewFolderIng"
          ...
          <el-col :span="24"
            ...
            <!-- <el-tag class="choose-tag"
                    v-for="(item, index) in chooseFolders"
                    v-show="!lockFolder || item.lock"
                    @click.native.self="$lockFolder(item)"
                    :type="item.cfid === (chooseFolder && chooseFolder.cfid) ? 'danger' : 'info'"
                    :key="index">
              {{item.cfname}}
            </el-tag> -->
            <el-popover placement="bottom"
              ...
              <el-button class="button-new-tag"
                         ...
                         size="small"
                         icon="el-icon-plus"
                         @click="$chooseNewFolderVisible">新文件夹</el-button>
              <el-button class="button-new-tag"
                         slot="reference"
                         size="small"
                         icon="el-icon-refresh"
                         v-show="lockFolder"
                         @click="$clearLock">显示全部文件夹</el-button>
            </el-popover>
          </el-col>
        </el-row>
        <mk-hr></mk-hr>
          ...
          <li v-for="(file, index) in chooseFiles"
            ...
            <span class="el-upload-list__item-actions">
              <span class="el-upload-list__item-preview"
                    v-if="preivewSrc === file.url && previewPause === false"
                    @click="$handleViewPause()">
                <i class="el-icon-erp-pause"></i>
              </span>
              <span class="el-upload-list__item-preview"
                    v-else
 ...
 *
 * 文件名不支持，因此使用text模式展示文件时，新上传的文件没有问题，但初始化的文件的文件名是空的
 *
 */
