...
export const UploaderBuilder = (function() {
  ...
  UploaderBuilder.prototype.interceptor = function(interceptor) {
    this._interceptors.push(
      Object.assign(new SimpleUploadInterceptor(), interceptor)
    );
    return this;
  };
  /**
...
let ChunkUploadPattern = (function() {
    ...
    for (let _i = 0, _a = task.blocks; _i < _a.length; _i++) {
      ...
      let loop1 = function(chunk) {
        chain = chain.then(function() {
          return _this.uploadChunk(chunk, _this.uploader._token, task);
        });
      };
      for (let _b = 0, _c = block.chunks; _b < _c.length; _b++) {
        let chunk = _c[_b];
  ...
  ChunkUploadPattern.prototype.uploadChunk = function(chunk, token, task) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      // 前一个chunk,如果存在的话
      let prevChunk = isFirstChunkInBlock
        ? null
        : chunk.block.chunks[chunkIndex - 1];
      let url = isFirstChunkInBlock
        ? _this.getUploadBlockUrl(chunk.block.data.size)
        : _this.getUploadChunkUrl(
            chunk.start,
      ...
      xhr.setRequestHeader('Mime', task.file.type); // 添加Mime信息
      xhr.setRequestHeader('Authorization', token); // 添加token验证头
      // 分片上传中
      xhr.upload.onprogress = function(e) {
        if (e.lengthComputable) {
          let progress = Math.round(
            ((task.finishedBlocksSize + chunk.start + e.loaded) /
              task.file.size) *
              100
          );
          if (task.progress < progress) {
      ...
      xhr.upload.onload = function() {
        let progress = Math.round(
          ((task.finishedBlocksSize + chunk.start + chunk.data.size) /
            task.file.size) *
            100
        );
        if (task.progress < progress) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          if (xhr.status == 200 && xhr.responseText != '') {
            ...
            let hasNextChunkInThisBlock =
              chunkIndex1 != chunk.block.chunks.length - 1;
  ...
  ChunkUploadPattern.prototype.getUploadChunkUrl = function(start, ctx, host) {
    return (host || this.uploader.domain) + '/bput/' + ctx + '/' + start + '/';
  };
  /**
   * 获取合并块为文件的url
   * @param fileSize 文件大小
  ...
  ChunkUploadPattern.prototype.getMakeFileUrl = function(fileSize, encodedKey) {
    if (encodedKey) {
      return (
        this.uploader.domain + '/mkfile/' + fileSize + '/key/' + encodedKey
      );
    } else {
...
export const Uploader = (function() {
  function Uploader(builder) {
    ...
    this.handleFiles = function() {
      ...
      // 从任务队列中去除任务
      for (
        let _d = 0, interceptedTasks1 = interceptedTasks;
        _d < interceptedTasks1.length;
        _d++
      ) {
  ...
  Uploader.isChunkTask = function(task) {
    return (
      task.constructor.name === ChunkTask.name && task instanceof ChunkTask
    );
  };
  /**
   * 是否是直传任务
  ...
  Uploader.isDirectTask = function(task) {
    return (
      task.constructor.name === DirectTask.name && task instanceof DirectTask
    );
  };
  /**
   * 生成task
