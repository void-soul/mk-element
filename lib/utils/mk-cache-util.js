...
const _set = (key, value) => {
  if (productMode === true) {
    let randomKey = guid();
    let saveTime = Number(new Date());
    let signToken = md5(key + saveTime + randomKey + value + _SCRIT);
...
const _get = (key) => {
  ...
  if (value && productMode === true) {
    ...
    if (value.length === 4) {
      let signToken = value[0];
      let saveTime = value[1];
      let randomKey = value[2];
      let value_ = value[3];
      let sign = md5(key + saveTime + randomKey + value_ + _SCRIT);
